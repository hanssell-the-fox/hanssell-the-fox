<img src="./images/forest.jpg" width="100%" alt="A drawing of a colorfull forest"/>
&nbsp;

<!-- Resume opening -->
<div align="center">

**Tecnologia**, **desenvolvimento de software** e **novações**. Tudo em nosso mundo atual faz minha imaginação voar admirando as possibilidades do que se pode fazer e como o futuro, provavelmente, será.

</div>
&nbsp;

<!-- Section separator -->
<div align="center">
  <img width="30" src="./images/fox.svg" alt="a fox face">
  <br/><br/>
</div>

<!-- Sticker at the right -->
<img align="right" src="./images/sticker.png" width="50%" alt="a sticker of a fox"/>

<!-- A simple list of what I like and goals -->
<div>

:fox_face:&nbsp; **Wederson de Oliveira Calixto**, também, conhecido como **Hanssell a raposa** nas redes sociais e por amigos. <br/>

:fox_face:&nbsp; Estudante de **Engenharia de Software**, focado em **Desenvolvimento Web**, **Qualidade de software** e **Segurança de processos**.<br/>

:bookmark:&nbsp; Sempre leio artigos e livros sobre **desenvolvimento de software** e **boas práticas**<br/>

:bookmark:&nbsp; Estudo línguas estrangeiras como um passatempo. <br/>

:two_hearts:&nbsp; Gosto de usar qualquer qualquer tipo de tecnologia quando estou praticando em meus projetos pessoais. <br/>

:two_hearts:&nbsp;Admiro **raposas**, amo um bom **café** :coffee: e **aprender** coisas novas. <br/>

</div>

<!-- Contact links -->
<br/>
<div align="center">

[![Linkedin](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white 'LinkedIn')](https://linkedin.com/in/wederson-calixto)&nbsp;[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:the-fox@hanssell.dev)

</div>
